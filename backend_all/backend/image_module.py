...
def model_init():
    ...
    inference_model.load_state_dict(torch.load(config.BEST_MODEL_PATH, map_location=device))

    return inference_model, classes, device
...
if __name__ == '__main__':
    ...
    # 数据路径
    train_dir = "image_recognition/mineral_dataset/train"
    val_dir = "image_recognition/mineral_dataset/test"
    ...
    # 重新加载模型进行推理
    inference_model = load_model().to(device)
    inference_model.load_state_dict(torch.load("image_recognition/myMineralModel/convit_mineral.pth", map_location=device))

    # 测试图片分类
    test_image_path = "image_recognition/testImgMineral/sodalite.jpeg"
